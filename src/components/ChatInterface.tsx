...
import { SendIcon, MapPinIcon, Globe, Bot } from 'lucide-react';
...
const ChatInterface = ({ mapsApiKey, travelApiKey, travelApiEndpoint }: ChatInterfaceProps) => {
  ...
  useEffect(() => {
    const welcomeMessage: Message = {
      id: 'welcome',
      content: "👋 Hello! I'm your AI travel assistant. Ask me anything about navigation, translations, or local recommendations!",
      sender: 'bot',
      timestamp: new Date(),
      type: 'text'
    };
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const isLocationRequest = inputValue.toLowerCase().includes('where') || 
      ...
      const isTranslationRequest = inputValue.toLowerCase().includes('translate') || 
                                  inputValue.toLowerCase().includes('language');
      ...
      if (isLocationRequest) {
        ...
        botResponse = {
          id: (Date.now() + 1).toString(),
          content: "I'm your AI travel assistant. I can help with travel recommendations, language translation, and navigation. What would you like to know?",
          sender: 'bot',
          timestamp: new Date(),
          type: 'text'
        };
